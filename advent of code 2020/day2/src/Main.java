public class Main {
    ...
    //function to parse Input string to lowest number, highest number, character , password
    public static void parseInput(String input){

        int count =0;
        String[] inputStrings= input.split("\\n");
        for(int i=0; i<inputStrings.length;i++){
            String[] passwordPolicy= inputStrings[i].split("[- :  ]");
//            String passwordPolicy= inputStrings[i];
            int lowest = Integer.parseInt(passwordPolicy[0]);
            int highest = Integer.parseInt(passwordPolicy[1]);
            String policy = passwordPolicy[2];
            String password = passwordPolicy[4];
            //call function that check if password is valid
            if(isValid(lowest,highest,policy,password)){
                count++;
            }
    ...
    public static boolean isValid(int first, int second, String charPolicy, String password) {
        boolean fir = false;
        boolean sec = false;
        for(int i =0; i<password.length(); i++){
            if(charPolicy.charAt(0)==password.charAt(first-1) ) fir = true;
            if(charPolicy.charAt(0)==password.charAt(second-1)) sec = true;
        }
